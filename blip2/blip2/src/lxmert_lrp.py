...
from blip2.blip2.src.layers import *
...
@dataclass
class blip2ModelOutput(ModelOutput):
    """
    blip2's outputs that contain the last hidden states, pooled outputs, and attention probabilities for the language,
    visual, and, cross-modality encoders. (note: the visual encoder in blip2 is referred to as the "relation-ship"
    encoder")
...
@dataclass
class blip2ForQuestionAnsweringOutput(ModelOutput):
    """
    Output type of :class:`~transformers.blip2ForQuestionAnswering`.
...
@dataclass
class blip2ForPreTrainingOutput(ModelOutput):
    """
    Output type of :class:`~transformers.blip2ForPreTraining`.
...
def load_tf_weights_in_blip2(model, config, tf_checkpoint_path):
...
class blip2Embeddings(nn.Module):
...
class blip2AttentionOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class blip2CrossAttentionLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.att = blip2Attention(config)
        self.output = blip2AttentionOutput(config)
        self.clone = Clone()
...
class blip2SelfAttentionLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.self = blip2Attention(config)
        self.output = blip2AttentionOutput(config)
        self.clone = Clone()
...
class blip2Intermediate(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class blip2Layer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.attention = blip2SelfAttentionLayer(config)
        self.intermediate = blip2Intermediate(config)
        self.output = blip2Output(config)
        self.clone = Clone()
...
class blip2XLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        # The cross-attention Layer
        self.visual_attention = blip2CrossAttentionLayer(config)

        # Self-attention Layers
        self.lang_self_att = blip2SelfAttentionLayer(config)
        self.visn_self_att = blip2SelfAttentionLayer(config)

        # Intermediate and Output Layers (FFNs)
        self.lang_inter = blip2Intermediate(config)
        self.lang_output = blip2Output(config)
        self.visn_inter = blip2Intermediate(config)
        self.visn_output = blip2Output(config)
...
class blip2VisualFeatureEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class blip2Encoder(nn.Module):
    def __init__(self, config):
        ...
        self.visn_fc = blip2VisualFeatureEncoder(config)
        self.config = config
        ...
        self.layer = nn.ModuleList([blip2Layer(config) for _ in range(self.num_l_layers)])
        self.x_layers = nn.ModuleList([blip2XLayer(config) for _ in range(self.num_x_layers)])
        self.r_layers = nn.ModuleList([blip2Layer(config) for _ in range(self.num_r_layers)])
...
class blip2Pooler(nn.Module):
    def __init__(self, config):
        super(blip2Pooler, self).__init__()
        self.dense = Linear(config.hidden_size, config.hidden_size)
...
class blip2PredictionHeadTransform(nn.Module):
    def __init__(self, config):
        super(blip2PredictionHeadTransform, self).__init__()
        self.dense = Linear(config.hidden_size, config.hidden_size)
...
class blip2LMPredictionHead(nn.Module):
    def __init__(self, config, blip2_model_embedding_weights):
        super(blip2LMPredictionHead, self).__init__()
        self.transform = blip2PredictionHeadTransform(config)

        # The output weights are the same as the input embeddings, but there is
        # an output-only bias for each token.
        self.decoder = Linear(
            blip2_model_embedding_weights.size(1),
            blip2_model_embedding_weights.size(0),
            bias=False,
        )
        self.decoder.weight = blip2_model_embedding_weights
        self.bias = nn.Parameter(torch.zeros(blip2_model_embedding_weights.size(0)))
...
class blip2VisualAnswerHead(nn.Module):
...
class blip2VisualObjHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.transform = blip2PredictionHeadTransform(config)
...
class blip2PreTrainingHeads(nn.Module):
    def __init__(self, config, blip2_model_embedding_weights):
        super(blip2PreTrainingHeads, self).__init__()
        self.predictions = blip2LMPredictionHead(config, blip2_model_embedding_weights)
...
class blip2PreTrainedModel(PreTrainedModel):
    ...
    load_tf_weights = load_tf_weights_in_blip2
...
blip2_START_DOCSTRING = r"""

    The blip2 model was proposed in `blip2: Learning Cross-Modality Encoder Representations from Transformers
    ...
    Parameters:
        config (:class:`~transformers.blip2Config`): Model configuration class with all the parameters of the model.
            Initializing with a config file does not load the weights associated with the model, only the
            configuration. Check out the :meth:`~transformers.PreTrainedModel.from_pretrained` method to load the model
            weights.
"""

blip2_INPUTS_DOCSTRING = r"""

    Args:
        input_ids (:obj:`torch.LongTensor` of shape :obj:`({0})`):
            Indices of input sequence tokens in the vocabulary.

            Indices can be obtained using :class:`~transformers.blip2Tokenizer`. See
        ...
        visual_pos: (:obj:`torch.FloatTensor` of shape :obj:՝(batch_size, num_visual_features, visual_pos_dim)՝):
            ...
            pre-trained blip2 model expects these spacial features to be normalized bounding boxes on a scale of 0 to
            1.
...
@add_start_docstrings(
    "The bare blip2 Model transformer outputting raw hidden-states without any specific head on top.",
    blip2_START_DOCSTRING,
)
class blip2Model(blip2PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.embeddings = blip2Embeddings(config)
        self.encoder = blip2Encoder(config)
        self.pooler = blip2Pooler(config)
        self.init_weights()
    ...
    @add_start_docstrings_to_model_forward(blip2_INPUTS_DOCSTRING.format("batch_size, sequence_length"))
    @add_code_sample_docstrings(
        processor_class=_TOKENIZER_FOR_DOC,
        checkpoint="Salesforce/blip2-opt-2.7b",
        output_type=blip2ModelOutput,
...
@add_start_docstrings(
    """blip2 Model with a specified pretraining head on top. """,
    blip2_START_DOCSTRING,
)
class blip2ForPreTraining(blip2PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        # Configuration
        self.config = config
        ...
        # Pre-training heads
        self.cls = blip2PreTrainingHeads(config, self.blip2.embeddings.word_embeddings.weight)
        if self.task_obj_predict:
            self.obj_predict_head = blip2VisualObjHead(config)
        if self.task_qa:
            self.answer_head = blip2VisualAnswerHead(config, self.num_qa_labels)
    ...
    def get_qa_logit_layer(self) -> nn.Module:
        ...
        Returns:
            ...
            blip2 does not have a visual answering head.
        """
    ...
    @add_start_docstrings_to_model_forward(blip2_INPUTS_DOCSTRING.format("batch_size, sequence_length"))
    @replace_return_docstrings(output_type=blip2ForPreTrainingOutput, config_class=_CONFIG_FOR_DOC)
    def forward(
            self,
            input_ids=None,
            visual_feats=None,
        ...
        return blip2ForPreTrainingOutput(
            loss=total_loss,
            prediction_logits=lang_prediction_scores,
            cross_relationship_score=cross_relationship_score,
            question_answering_score=answer_score,
            language_hidden_states=blip2_output.language_hidden_states,
            vision_hidden_states=blip2_output.vision_hidden_states,
            language_attentions=blip2_output.language_attentions,
            vision_attentions=blip2_output.vision_attentions,
            cross_encoder_attentions=blip2_output.cross_encoder_attentions,
        )



@add_start_docstrings(
    """blip2 Model with a visual-answering head on top for downstream QA tasks""",
    blip2_START_DOCSTRING,
)
class blip2ForQuestionAnswering(blip2PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        # Configuration
        self.config = config
    ...
    def get_qa_logit_layer(self) -> nn.Module:
        ...
        Returns:
            ...
            NoneType object if blip2 does not have the visual answering head.
        """
    ...
    @add_code_sample_docstrings(
        processor_class=_TOKENIZER_FOR_DOC,
        checkpoint="Salesforce/blip2-opt-2.7b",
        output_type=blip2ForQuestionAnsweringOutput,
        config_class=_CONFIG_FOR_DOC,
    )
    def forward(
            self,
        ...
        if not return_dict:
            output = (answer_score,) + blip2_output[3:]
            return (loss,) + output if loss is not None else output

        self.vis_shape = blip2_output.vision_output.shape

        return blip2ForQuestionAnsweringOutput(
            loss=loss,
            question_answering_score=answer_score,
            language_hidden_states=blip2_output.language_hidden_states,
            vision_hidden_states=blip2_output.vision_hidden_states,
    ...
    def relprop(self, cam, **kwargs):
        ...
        cam_lang, cam_vis = self.blip2.relprop((cam_lang, cam_vis), **kwargs)
        return cam_lang, cam_vis